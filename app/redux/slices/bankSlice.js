...
export const fetchBanks = createAsyncThunk('banks/fetchBanks', async () => {
  const response = await axios.get('https://12flf535-3012.inc1.devtunnels.ms/api/bank');
...
export const addBank = createAsyncThunk('banks/addBank', async (bankData) => {
  ...
  const response = await axios.post('https://12flf535-3012.inc1.devtunnels.ms/api/bank', bankData);
...
export const updateBank = createAsyncThunk('banks/updateBank', async ({ id, bankData }) => {
  const response = await axios.put(`https://12flf535-3012.inc1.devtunnels.ms/api/bank/${id}`, bankData);
  console.log(response.data);
...
export const deleteBank = createAsyncThunk('banks/deleteBank', async (id) => {
  await axios.delete(`https://12flf535-3012.inc1.devtunnels.ms/api/bank/${id}`);
  return id;
});
