...
const TransactionTable1 = ({ onEdit }) => {
      ...
      <table className="min-w-full bg-white border">
        <thead>
          ...
          <th className="p-2 border">Sr</th>
            ...
            <th className="p-2 border">Shop Before Balance</th>
            ...
            <th className="p-2 border">Shop After Balance</th>
        ...
        <tbody>
          {transactions?.transactions?.map((transaction,index) => (
            <tr key={transaction.id}>
              ...
              <td className="p-2 border  text-center">{transaction.MasterModel?.name}</td>
              <td className="p-2 border  text-center">{transaction.remark}</td>
              <td className="p-2 border  text-center">{transaction.ShopBeforeBalances}</td>
              {/* <td className="p-2 border  text-center">{transaction.BeforeBalances}</td> */}
              <td className="p-2 border  text-center">{transaction.type === "Credit" ? transaction.amount : 0}</td>
              <td className="p-2 border  text-center">{transaction.type !== "Credit" ? transaction.amount:0}</td>
              {/* <td className="p-2 border  text-center">{transaction.AfterBalances}</td> */}
              <td className="p-2 border  text-center">{transaction.ShopAfterBalances}</td>
